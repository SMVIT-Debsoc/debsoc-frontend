...
function LocalMockDrillWorkspace({ embedded = false }: { embedded?: boolean }) {
  ...
  return (
    <div className={`w-full min-w-0 space-y-5 ${embedded ? "min-h-[620px]" : "mx-auto max-w-6xl"}`}>
      {!embedded && <><LocalWorkspaceHeader title="Mock Drill" subtitle="Practice an argument, then review a structured debate analysis." /><HowToUseCard kind="drill" /></>}
        ...
        <Card className="min-h-[560px] p-4 sm:p-6">
          ...
          <div className="mt-5 space-y-4">
            ...
            <div className="flex flex-wrap items-center justify-between gap-3 border-t border-slate-200 pt-4 dark:border-white/10">
              ...
              <PrimaryButton type="button" onClick={analyze} disabled={!response.trim() || loading}>{loading ? <><Loader2 size={15} className="motion-safe:animate-spin" aria-hidden="true" /> Analyzing locally...</> : <><Sparkles size={15} aria-hidden="true" /> Analyze response</>}</PrimaryButton>
            </div>
          </div>
        </Card>
