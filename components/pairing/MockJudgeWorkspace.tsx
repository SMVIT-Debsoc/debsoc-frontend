...
function LocalMockJudgeWorkspace({ embedded = false }: { embedded?: boolean }) {
  ...
  return (
    <div className={`w-full min-w-0 space-y-5 ${embedded ? "min-h-[620px]" : "mx-auto max-w-6xl"}`}>
      {!embedded && <><LocalWorkspaceHeader title="Mock Judge" subtitle="Review a speech with a structured judging-style feedback surface." /><HowToUseCard kind="judge" /></>}
      ...
      <div className="grid gap-5 xl:grid-cols-[minmax(0,1.35fr)_minmax(280px,.7fr)]">
        <Card className="min-h-[560px] p-4 sm:p-6">
          ...
          <div className="mt-5 space-y-4">
            ...
            <div className="flex flex-wrap items-center justify-between gap-3 border-t border-slate-200 pt-4 dark:border-white/10"><SecondaryButton type="button" onClick={reset} disabled={!motion && !speakerInfo && !argument && !result && !loading}><RotateCcw size={15} aria-hidden="true" /> Reset</SecondaryButton><PrimaryButton type="button" onClick={judge} disabled={!argument.trim() || loading}>{loading ? <><Loader2 size={15} className="motion-safe:animate-spin" aria-hidden="true" /> Judging locally...</> : <><Sparkles size={15} aria-hidden="true" /> Review speech</>}</PrimaryButton></div>
          </div>
        </Card>
