...
function SessionDetails({
    ...
    const presentEntries = attendanceEntries.filter((entry) =>
        isPresentStatus(entry.status),
    );
    const presentCount = presentEntries.length;
    const myAttendance =
        attendanceHistory.find((item) => item.session.id === session.id) ??
        null;
