...
export default function RealMockJudgeWorkspace({ embedded = false }: { embedded?: boolean }) {
  ...
  const judge = async () => {
    ...
    try {
      setResult(await debassClient.judge(acceptedKey, argument.trim(), controller.signal, model));
    } catch (caught) {
