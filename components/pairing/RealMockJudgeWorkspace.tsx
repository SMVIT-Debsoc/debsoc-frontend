...
import AnalysisTransitionLayout, { AnalysisResultReveal } from "./AnalysisTransitionLayout";
  ...
  return (
    <div className={`w-full min-w-0 space-y-5 ${embedded ? "min-h-[620px]" : "mx-auto max-w-6xl"}`}>
      ...
      <AnalysisTransitionLayout analysisActive={analysisActive} stacked={embedded}>
        ...
        <Card className="min-h-[560px] p-4 sm:p-6"><div className="flex items-center gap-2"><Gavel size={16} className="text-indigo-600 dark:text-indigo-300" aria-hidden="true" /><div><h2 className="text-base font-semibold text-slate-950 dark:text-slate-100">Judge Feedback</h2><p className="mt-1 text-sm text-slate-500 dark:text-slate-400">Scores and feedback appear only when returned by Debass.</p></div></div><div className="mt-5" aria-busy={loading}>{loading ? <div className="flex min-h-[390px] flex-col items-center justify-center text-center" role="status" aria-live="polite"><Loader2 size={28} className="motion-safe:animate-spin text-indigo-600 dark:text-indigo-300" aria-hidden="true" /><p className="mt-4 text-sm font-medium text-slate-700 dark:text-slate-300">Judging…</p><p className="mt-1 text-xs text-slate-500 dark:text-slate-400">Debass is reviewing the argument.</p></div> : result ? <AnalysisResultReveal contentKey="judge-result"><div className="space-y-4"><div className="rounded-xl border border-indigo-200 bg-indigo-50/70 p-4 dark:border-indigo-400/20 dark:bg-indigo-400/[0.08]"><div className="flex items-center justify-between gap-3"><span className="text-xs font-semibold uppercase tracking-[0.14em] text-indigo-800 dark:text-indigo-300">Returned score</span><span className="text-2xl font-semibold text-indigo-900 dark:text-indigo-200">{result.score}</span></div></div><section className="rounded-xl border border-slate-200 p-4 dark:border-white/10"><h3 className="text-xs font-semibold uppercase tracking-[0.14em] text-slate-500 dark:text-slate-400">Reasoning</h3><div className="mt-3"><LocalMarkdown content={result.reasoning} /></div></section><div className="grid gap-3 sm:grid-cols-2"><FeedbackList title="Strengths" items={result.strengths} tone="emerald" /><FeedbackList title="Weaknesses" items={result.weaknesses} tone="red" /></div><SecondaryButton type="button" onClick={() => void judge()} disabled={loading || keyState !== "valid"} className="w-full"><RotateCcw size={14} aria-hidden="true" /> Retry review</SecondaryButton></div></AnalysisResultReveal> : error ? <AnalysisResultReveal contentKey="judge-error"><div className="space-y-3 rounded-xl border border-red-200 bg-red-50 p-4 dark:border-red-400/20 dark:bg-red-400/[0.08]" role="alert"><p className="text-sm font-medium text-red-900 dark:text-red-200">{error}</p><SecondaryButton type="button" onClick={() => void judge()} disabled={loading || keyState !== "valid"}><RotateCcw size={14} aria-hidden="true" /> Retry</SecondaryButton></div></AnalysisResultReveal> : <EmptyState title="Judge Feedback will appear here" body="Add an argument, validate your key, then choose Review argument." />}</div></Card>
