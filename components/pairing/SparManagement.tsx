      ...
      <div className="grid gap-5 lg:grid-cols-[minmax(0,1.1fr)_minmax(320px,0.9fr)]">
        <Card className="p-4 sm:p-5">
          ...
          <form onSubmit={submit} className="grid gap-4 sm:grid-cols-2">
            <Field label="Date"><input className="form-input" type="date" value={sparDate} max={todayInputValue()} onChange={(event) => setSparDate(event.target.value)} required /></Field>
            <Field label="Motion Type"><input className="form-input" value={motionType} onChange={(event) => setMotionType(event.target.value)} required /></Field>
            <Field label="Motion Text"><input className="form-input" value={motionText} onChange={(event) => setMotionText(event.target.value)} /></Field>
            <Field label="BP Position"><select className="form-input" value={bpPosition} onChange={(event) => setBpPosition(event.target.value as typeof bpPosition)}>{benchPositions.map((position) => <option key={position}>{position}</option>)}</select></Field>
            <Field label="Teammate"><select className="form-input" value={isIronMan ? "iron" : teammateKey} onChange={(event) => { const value = event.target.value; setIsIronMan(value === "iron"); setTeammateKey(value === "iron" ? "" : value); }}><option value="">Select teammate</option><option value="iron">Iron Man</option>{teammateOptions.map((option) => <option key={option.key} value={option.key}>{option.label}</option>)}</select></Field>
            {!isIronMan && <Field label="Speaking Role"><select className="form-input" value={selectedRole} onChange={(event) => setSelectedRole(event.target.value as typeof selectedRole)}>{rolesForPosition.map((role) => <option key={role}>{role}</option>)}</select></Field>}
            <Field label={isIronMan ? `${rolesForPosition[0]} Score` : "Speaker Score"}><input className="form-input" type="number" min="50" max="100" step="0.1" value={firstScore} onChange={(event) => setFirstScore(event.target.value)} required /></Field>
            {isIronMan && <Field label={`${rolesForPosition[1]} Score`}><input className="form-input" type="number" min="50" max="100" step="0.1" value={secondScore} onChange={(event) => setSecondScore(event.target.value)} required /></Field>}
            <Field label="Team Rank"><select className="form-input" value={teamRank} onChange={(event) => setTeamRank(event.target.value)}>{[1, 2, 3, 4].map((rank) => <option key={rank} value={rank}>{rank}</option>)}</select></Field>
            <div className="sm:col-span-2"><PrimaryButton type="submit" disabled={submitting || (!isIronMan && !teammateKey)}>{submitting ? "Submitting..." : "Submit Spar"}</PrimaryButton></div>
          </form>
        </Card>
