...
import { regenerateProposal } from "@/lib/server/pairing/review";
...
export async function POST(_: Request, { params }: { params: Promise<{ proposalId: string }> }) {
  ...
  try { return ok(await regenerateProposal({ proposalId: parsedParams.data.proposalId, reviewerId: sessionResult.user.id })); } catch (caught) { return error(caught instanceof Error ? caught.message : "Proposal regeneration failed", 400); }
}
