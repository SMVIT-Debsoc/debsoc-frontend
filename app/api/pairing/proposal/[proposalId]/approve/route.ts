...
import { approveProposal } from "@/lib/server/pairing/review";
...
export async function POST(_: Request, { params }: { params: Promise<{ proposalId: string }> }) {
  ...
  try { return ok(await approveProposal(parsedParams.data.proposalId, sessionResult.user.id)); } catch (caught) { return error(caught instanceof Error ? caught.message : "Proposal approval failed", 400); }
}
